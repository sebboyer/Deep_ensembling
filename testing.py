...
'''
def cycle_train_and_test(individual_models,concat_models,DataSets,network,X_test,y_test,X_val_list,y_val_list,skip_models,target):
...
def test_models_pb(individual_models,concat_models,DataSets,X_test_list,y_test_list,X_train_list,y_train_list,folder,network_name,skip_models=[]):
    ...
    for target in DataSets:
        ...
        X_val_list = [X_test_list[s] for s in DataSets if s!=target]
        y_val_list = [y_test_list[s] for s in DataSets if s!=target]
        
        network = load_net(folder,network_name)
        result_target = cycle_train_and_test(individual_models,concat_models,DataSets,network,X_test,y_test,X_val_list,y_val_list,skip_models,target)
...
def test_all_models(individual_models,feat_dic_list,featids_feat,pb_list,structure_folder,networks,concat_folder,para=0,skip_models=[],DataSets=[]):
    ...
    def test_models_pb_aux(individual_models,feat_dic_list,featids_feat,structure_folder,networks,concat_folder,pb):
        ...
        for network_name in networks:
            ...
            result_pb[network_name] = test_models_pb(individual_models_pb,concat_models_pb,DataSets,X_test_list,y_test_list,X_train_list,y_train_list,structure_folder,network_name,skip_models)
    ...
    if para==1:
        ...
        results_list = []
        for pb in pb_list:
            results_list.append(test_models_pb_aux(individual_models,feat_dic_list,featids_feat,structure_folder,networks,concat_folder,pb))
